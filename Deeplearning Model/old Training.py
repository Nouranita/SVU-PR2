...
# Config (EDIT THESE)
# =========================
ROOT_DIR = r"C:\Users\Nouran\Desktop\PR2\Data Set Patterned"   # <-- dataset root
...
# checkpointing
OUT_DIR = "runs_effnetv2s_lung"
SAVE_EVERY = 5  # epochs
BEST_METRIC = "f1_macro"  # choose: f1_macro or acc or f1_weighted

# For x-rays: keep grayscale, but EfficientNet expects 3 channels; replicate 1->3
USE_GRAYSCALE = True
...
def to_3ch(t):
    # t: (C,H,W); t is a Tensor (C,H,W). If grayscale -> C=1, replicate to C=3
...
def get_transforms(img_size=224, train=True, force_3ch=True):
    ...
    # For medical imaging, avoid aggressive color jitter/hue.
    # Use mild geometric augmentation and slight contrast/brightness changes.
    if train:
        ops = [
...
def main():
    ...
    # reuse last fitted temperature from best epoch would be ideal, but for simplicity I fit on val set once more using the best model:
